...
BEGIN
   ...
   SELECT pdt_code, pdt_sht_desc, pdt_description, pdt_unt_code, pdt_loc_code, pdt_sloc_code, 
		    ...
		    pdt_delete_date,pdt_merge_qty,cat_sht_desc,loc_sht_desc,sloc_sht_desc,unt_sht_desc,VAT_DESCRIPTION,VAT_RATE
    ...
    left join inv_vat on VAT_ID=pdt_vat_id	
     WHERE ((pdt_sht_desc LIKE CONCAT('%',REPLACE(p_search_data,'"',''),'%'))
              OR (pdt_description LIKE CONCAT('%',REPLACE(p_search_data,'"',''),'%'))
              OR (cat_description LIKE CONCAT('%',REPLACE(p_search_data,'"',''),'%'))
              OR (p_search_data IS NULL)
             )
